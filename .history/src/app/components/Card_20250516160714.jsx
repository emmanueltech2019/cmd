...
const Card = ({
    ...
    outlineElement,
    button,
    checkers = [{title: '', check: medellinCheck}, {title: '', check: medellinCheck}, {title: '', check: medellinCheck}, {title: '', check: medellinCheck}, {title: '', check: medellinCheck}]
}) => {
  return (
        <div className={`${optionalClass || 'border-4 border-white bg-web-cardbgcolor p-web-card rounded-web-cardradius'}`}>
            ...
            >
                ...
                {outlineElement}
                {button}
                {checkers.map((items, index) => (
                    (
                        <div key={index} className="flex flex-col gap-3">
                            <div className="flex items-center gap-2">
                                <Image src={items.check} width={100} height={100} alt='check' />
