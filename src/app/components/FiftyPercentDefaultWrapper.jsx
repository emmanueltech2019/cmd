...
const FiftyPercentDefaultWrapper = ({ className = '', extraClass = '', inlineClass = '', outlineText, title, description, textClass, infiniteSlider }) => {
  return (
    <div className={`${className || `flex flex-col items-center justify-center md:w-[50%] w-full mx-auto ${inlineClass}`}`}>
      ...
      <h1 className={`md:my-5 my-3 md:px-5 text-center font-light text-web-blue text-web-contactformfont ${textClass}`}>{title}</h1>
      <p className={`md:px-5 px-10 text-lg font-light text-center text-web-lightblue ${textClass}`}>{description}</p>
      {infiniteSlider}
    </div>
  )
}
